...
JdbcTemplate jdbc;
	...
	public int add(Cart cart) throws DataAccessException{
		String sql = "insert into orderdetails(cartId,userId,dishId,dishName,description,type,price) values (?,?,?,?,?,?,?)";
		Dish dish= cart.getDish();
		int response = jdbc.update(sql,cart.getCartId(),cart.getUserId(),dish.getDishId(),dish.getDishName(),dish.getDescription(),dish.getType(),dish.getPrice() );
		return response;
	}
